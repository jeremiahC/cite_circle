...
$(function(){
	...
	$('#personal_submits').submit(function(){
	...
	console.log(form_data);
		  ...
		  $.ajax({
		        url: "<?php echo base_url();?>ProfileController/edit_profile",
		        type: 'POST',
		        data: form_data,
		        success: function(msg) {
		            if (msg =='true')
		            {
		                $('#personal_load').html('<i style="color: green">Username is invalid</i>. <i class="big green checkmark icon"></i>');
		            }else{
		                $('#personal_load').html('<i style="color: green">Username is valid</i>. <i class="big green checkmark icon"></i>');
		        }
		        }
		    });
		    return false;
...
$(function(){
					   ...
					   success: function(msg){
					    ...
					    else
					    {
					     $("#password_verify").fadeIn(100).html('<i style="color: red">The password is already taken, please choose another password</i>. <i class="big red remove icon"></i>');
				  		...
				  		$("#password_verify").fadeIn(100).html('<i style="color: orange">Username is 6 four characters</i>. <i class="big yellow warning icon"></i>');
				  }
			});
	...
	$(function(){
		...
		$(function(){
				...
				$('#account_submit').mouseover(function(){
					...
					$.ajax({
						   type: "POST",
						   url: "<?php echo base_url();?>ProfileController/check_validation",
						   data: dataString,
						   success: function(msg){
						    if(msg == "true")
						    {
							    $("#account_validation").fadeIn(100).html('<i style="color: green">Your account is valid to edit</i>. <i class="big green checkmark icon"></i>');
						    }
						    else
						    {
						     $("#account_validation").fadeIn(100).html('<i style="color: red">Your account is not valid to edit</i>. <i class="big red remove icon"></i>');
						    }
						   }
						  });
			});
